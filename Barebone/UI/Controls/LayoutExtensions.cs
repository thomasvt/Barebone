...
namespace Barebone.UI.Controls
{
    ...
    internal static class LayoutExtensions
    {
        ...
        public static void DockLeft(this AabbI area, int width, int gap, out AabbI dock, out AabbI remainder)
        {
            ...
            dock = new AabbI(area.MinCorner.X, area.MinCorner.Y, area.MinCorner.X + width, area.MaxCornerIncl.Y);
            if (dock.Width < 0) dock = AabbI.Zero;
            remainder = new AabbI(area.MinCorner.X + width + gap, area.MinCorner.Y, area.MaxCornerIncl.X, area.MaxCornerIncl.Y);
        ...
        public static void DockTop(this AabbI area, int height, int gap, out AabbI dock, out AabbI remainder)
        {
            ...
            dock = new AabbI(area.MinCorner.X, area.MinCorner.Y, area.MaxCornerIncl.X, area.MinCorner.Y + height);
            if (dock.Height < 0) dock = AabbI.Zero;
            remainder = new AabbI(area.MinCorner.X, area.MinCorner.Y + height + gap, area.MaxCornerIncl.X, area.MaxCornerIncl.Y);
        ...
        {
            width = Math.Min(width, area.Width);
            dock = new AabbI(area.MaxCornerIncl.X - width, area.MinCorner.Y, area.MaxCornerIncl.X, area.MaxCornerIncl.Y);
            if (dock.Height < 0) dock = AabbI.Zero;
            remainder = new AabbI(area.MinCorner.X, area.MinCorner.Y, area.MaxCornerIncl.X - width - gap, area.MaxCornerIncl.Y);
            if (remainder.Width < 0) remainder = AabbI.Zero;
        }
        ...
        {
            height = Math.Min(height, area.Height);
            dock = new AabbI(area.MinCorner.X, area.MaxCornerIncl.Y - height, area.MaxCornerIncl.X, area.MaxCornerIncl.Y);
            if (dock.Height < 0) dock = AabbI.Zero;
            remainder = new AabbI(area.MinCorner.X, area.MinCorner.Y, area.MaxCornerIncl.X, area.MaxCornerIncl.Y - height - gap);
            if (remainder.Height < 0) remainder = AabbI.Zero;
        }
    }
}
