    ...
    public sealed class DynamicBVH<TTag>
    {
        ...
        public float AreaRatio
        {
            ...
            get
            {
                ...
                var rootPeri = rootNode.Aabb.Perimeter;
        ...
        public bool Move(int id, in Aabb aabb, Vector2 displacement)
        {
            ...
            var fatAabb = aabb.Grow(AABBExtendSize);

            // Predict AABB movement
            var d = displacement * AABBMultiplier;

            // pls can we make math types mutable this sucks.
            var l = fatAabb.MinCorner.X;
            var b = fatAabb.MinCorner.Y;
            var r = fatAabb.MaxCorner.X;
            var t = fatAabb.MaxCorner.Y;
            ...
            if (treeAabb.Contains(aabb))
            {
                ...
                var hugeAabb = new Aabb(
                                        fatAabb.MinCorner - growAmount,
                                        fatAabb.MaxCorner + growAmount);
        ...
        private void InsertLeaf(int leaf)
        {
            ...
            for (; ; )
            {
                ...
                var indexPeri = indexAabb.Perimeter;
                Aabb combinedAabb = default;
                combinedAabb = Aabb.Union(indexAabb, leafAabb);
                var combinedPeri = combinedAabb.Perimeter;
            ...
            newParentNode.Aabb = Aabb.Union(leafAabb, siblingNode.Aabb);
        ...
        {
            var cost = Aabb.Union(baseAabb, node.Aabb).Perimeter;
        ...
        private void Balance(int index)
        {
            while (index != ProxyFree)
            {
                ...
                indexNode.Aabb = Aabb.Union(child1Node.Aabb, child2Node.Aabb);
            ...
            while (count > 1)
            {
                ...
                for (var i = 0; i < count; ++i)
                {
                    ...
                    for (var j = i + 1; j < count; ++j)
                    {
                        ...
                        var cost = Aabb.Union(aabbI, aabbJ).Perimeter;
                ...
                parentNode.Aabb = Aabb.Union(child1Node.Aabb, child2Node.Aabb);
        ...
        public void ShiftOrigin(in Vector2 newOrigin)
        {
            ...
            {
                ref var node = ref _nodes[i];
                var lb = node.Aabb.MinCorner;
                var tr = node.Aabb.MaxCorner;
